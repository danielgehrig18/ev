...
def so3_log_torch(delta_R):
    ...
    w = 0.5 / sinc[:,None,None] * (delta_R - delta_R.transpose(-1,-2))
    w = w[...,[2,0,1],[1,2,0]]
...
def so3_J_l_inv_torch(r):
    ...
    norm = torch.norm(r, dim=-1, keepdims=True)
    k = (1 / norm**2 - (1 + torch.cos(norm)) / (2 * norm * torch.sin(norm)))
    J_inv = torch.eye(3, device=r.device) + 0.5 * r_hat + k * r_hat @ r_hat
    J_inv[norm < 1e-4] = torch.eye(3, device=r.device) + 0.5 * r_hat
    return J_inv

def so3_hat_torch(r):
    out = torch.zeros(shape=(r.shape[:-1] + (3,3)), device=r.device)
    out[...,2,1] = r[...,0]
    out[...,1,0] = r[...,2]
    out[...,0,2] = r[...,1]
...
def se3_log_torch(delta_T):
    ...
    delta_t = so3_J_l_inv_torch(delta_r) @ delta_T[...,:3, 3]

    return torch.cat([delta_r, delta_t], dim=-1)
